...
- **Back Office (Admin):** Vue 3 (TypeScript), Vite
...
- `price` (Integer): 단가
- `stockQuantity` (Integer): **[핵심]** 현재 가용 재고 수량. (추후 입/출고와 연계 필요)

---

## 📏 4. Core Business Rules (Immutable)
...
3. **확장성:** 모든 테이블은 등록일시(`createdAt`), 수정일시(`updatedAt`)를 포함하는 공통 감시(Audit) 필드를 고려함 (현재 `Item`에는 미구현, 추후 반영 예정).
